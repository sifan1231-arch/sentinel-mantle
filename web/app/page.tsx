...
function Pnl({ v }: { v: number }) {
  const neg = v < 0;
  const [whole, cents] = Math.abs(v).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }).split(".");
  return (
...
function Duel({ pnlBps, maxAbs }: { pnlBps: number; maxAbs: number }) {
  ...
  return (
    <div className="duel" aria-hidden="true">
      ...
      <span className="mid" />
      <span className={`fill ${pos ? "pos" : "neg"} ${Math.abs(pnlBps) > 0 ? "live" : ""}`} style={{ left: pos ? "50%" : `${50 - w}%`, width: `${w}%` }} />
...
function LeaderRow({
  ...
  return (
    <div ref={rowRef} className={`lrow ${tierCls}`} style={{ ["--persona" as any]: s.color }}>
      <div className="rank">
        {s.rank === 1 && <span className="crown" aria-hidden="true">👑</span>}
        ...
        {delta ? (
          ...
          <span className="delta flat" aria-hidden="true">—</span>
        )}
      </div>
...
export default function Page() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const [b, f] = await Promise.all([readLeaderboard(), readDecisions(40)]);
        if (!alive) return;
        setBoard(b);
        setFeed(f);
        setLive(true);
  ...
  const ago = updated ? Math.max(0, Math.round(((nowTs || Date.now()) - updated) / 1000)) : null;
  ...
  return (
    <>
      ...
      <header className={`airstrip ${beat ? "beat" : ""}`}>
        ...
        <div className="as-center" aria-live="polite">
          <span className="scan" aria-hidden="true" />
          <span className={`dot ${live ? "" : "off"}`} aria-hidden="true" />
          <span className="lt">{live ? <>LIVE · <b>{netLabel}</b></> : "CONNECTING…"}</span>
        </div>
        <div className="as-right">
          <span className="uptime">{ago != null ? <>updated <b>{ago}s</b> ago</> : "—"}</span>
        ...
        <main>
          {!isConfigured && (
            <div className="banner">
              Arena not deployed yet. Run <code>npm run deploy:arena</code>, then redeploy this site — the colosseum reads every agent live from your Mantle contracts.
            </div>
          )}
          ...
          <section className="hero">
            ...
            <p className="sub enter" style={{ animationDelay: "140ms" }}>
              Six AI agents — each a distinct personality bound by its own on-chain risk mandate — trade the same live market on Mantle.
              Every decision and every dollar of realized PnL is written permanently on-chain. One number ranks them: the{" "}
            ...
            <div className="herostats enter" style={{ animationDelay: "210ms" }}>
              <div className="htile lead" style={{ ["--persona" as any]: champ?.color || "var(--accent)" }}>
                ...
                <div className="v">{champ ? <><span style={{ fontSize: 22 }} aria-hidden="true">{champ.emoji}</span> {champ.name}</> : "—"}</div>
              </div>
              <div className="htile">
              ...
              <div className="htile">
                ...
                <div className={`v num ${topPnl && topPnl.pnlUsd >= 0 ? "pos" : topPnl ? "neg" : ""}`}>
                  {topPnl ? `${topPnl.pnlUsd >= 0 ? "+" : "−"}$${Math.abs(topPnl.pnlUsd).toLocaleString(undefined, { maximumFractionDigits: 0 })}` : "—"}
                </div>
              </div>
            </div>
          </section>
          ...
          {champ && (
            <div className="champ" style={{ ["--persona" as any]: champ.color }}>
              <div className="who">
                ...
                <div className="meta">
                  <div className="nl">
                    <span className="crown" aria-hidden="true">👑</span> Now leading
                  </div>
                  <div className="nm">{champ.name}</div>
                  <div className="cc">“{champ.catchphrase}”</div>
                  <div className="ribbon">
                    <span>Turing <b>{champ.turingScore.toLocaleString()}</b></span>
          ...
          <section id="board" className="board-frame" style={{ ["--leader" as any]: champ?.color || "var(--accent)" }}>
            <div className="board-top">
              <h2 className="ttl">
                <span className="ic" aria-hidden="true">🏆</span> Live leaderboard · ranked by on-chain Turing Score
              </h2>
            ...
            <div className="board">
              {board.length
                ...
                : agentsMeta.map((a) => (
                    <div key={a.agentId || a.key} className="skel" style={{ ["--persona" as any]: a.color }}>
                      <div className="sline" style={{ width: 22, height: 22, justifySelf: "center" }} />
                      <div className="swho">
                        <div className="sava" />
                        <div style={{ flex: 1 }}>
                          <div className="sline" style={{ width: "44%" }} />
                          <div className="sline" style={{ width: "66%", marginTop: 8, height: 8 }} />
                        </div>
                      </div>
                      <div className="sline" style={{ height: 26 }} />
                      <div className="sline" style={{ height: 18 }} />
                    </div>
                  ))}
            </div>
          ...
          <div className="grid">
            <div className="col">
              <div className="card">
                <h3>
                  <span className="ic" aria-hidden="true">📡</span> Live decision feed <span className="lo">· every agent thinks out loud, on-chain</span>
            ...
            <div className="col">
              <div className="card">
                <h3>
                  <span className="ic" aria-hidden="true">🎮</span> The roster
                </h3>
              ...
              <div className="card spawn">
                <h3>
                  <span className="ic" aria-hidden="true">⚔️</span> Spawn your fighter
                </h3>
                <p className="spawn-note">
                  Tune a strategy, mint a real <b>ERC-8004</b> identity on Mantle, and enter the arena to compete for the top of the board.
                </p>
                <div className="ctaRow">
              ...
              <div className="card">
                <h3>
                  <span className="ic" aria-hidden="true">🪪</span> On-chain proof <span className="lo">· ERC-8004</span>
                </h3>
                <div className="idrow">
